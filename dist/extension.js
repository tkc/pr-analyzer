...
var GitHubAPIClient = class {
   ...
   */
  async getPullRequests(owner, repo, token) {
    try {
      ...
      if (!response.ok) {
        return (0, import_neverthrow.err)(this.handleFetchError(response.status, "Error fetching pull requests"));
      }
      const data = await response.json();
      return (0, import_neverthrow.ok)(data);
    } catch (error) {
      return (0, import_neverthrow.err)(this.handleError(error, "Error fetching pull requests"));
    }
  }
  /**
   ...
   */
  async getPullRequestDiff(owner, repo, pullRequestNumber, token) {
    try {
      const headers = new Headers();
      ...
      if (!response.ok) {
        return (0, import_neverthrow.err)(this.handleFetchError(response.status, `Error fetching diff for PR #${pullRequestNumber}`));
      }
      const data = await response.text();
      return (0, import_neverthrow.ok)(data);
    } catch (error) {
      return (0, import_neverthrow.err)(this.handleError(error, `Error fetching diff for PR #${pullRequestNumber}`));
    }
  }
  /**
...
var reposFile = fs.readFileSync("repos.yml", "utf8");
var repos = load(reposFile);
var repoUrl = repos.repositories[0];
var urlParts = repoUrl.split("/");
var OWNER = urlParts[3];
var REPO = urlParts[4];
var gitHubAPIClient = new GitHubAPIClient();
async function getPullRequests(apiClient) {
  const result = await apiClient.getPullRequests(OWNER, REPO, GITHUB_TOKEN);
  return result.match(
    (pullRequests) => {
      return pullRequests.map((pr) => ({
        ...
        diff: {
          // Initialize diff stat with 0 lines
          addedLines: 0,
          deletedLines: 0,
          totalLines: 0
        }
    ...
    (error) => {
      if (error.type === "rateLimitExceeded") {
        console.warn("GitHub API rate limit exceeded. Please set GITHUB_TOKEN in .env to increase the limit.");
      } else if (error.type === "notFound") {
        console.error("Repository not found.");
      } else {
        console.error("Error fetching pull requests:", error.message, error.type);
      }
      return [];
    }
  );
}
async function getPullRequestDiff(apiClient, pullRequestNumber) {
  const result = await apiClient.getPullRequestDiff(OWNER, REPO, pullRequestNumber, GITHUB_TOKEN);
  return result.match(
    (diff) => diff,
    // Success case: return diff
    (error) => {
      if (error.type === "rateLimitExceeded") {
        console.warn("GitHub API rate limit exceeded. Please set GITHUB_TOKEN in .env to increase the limit.");
      } else if (error.type === "notFound") {
        console.error(`Pull Request #${pullRequestNumber} not found.`);
      } else if (error.type === "diffTooLarge") {
        console.warn(`Diff for PR #${pullRequestNumber} is too large to fetch.`);
        return "";
      } else {
        console.error(`Error fetching diff for PR #${pullRequestNumber}:`, error.message, error.type);
      }
      return null;
    }
  );
}
function parseDiff(diff, filename) {
  let codeAdditions = 0;
  let codeDeletions = 0;
  let docAdditions = 0;
  let docDeletions = 0;
  ...
  switch (fileExtension) {
    ...
    case "md":
    case "txt":
      docAdditions = additions;
      docDeletions = deletions;
      break;
    case "json":
    case "yml":
    case "yaml":
    case "ini":
      configAdditions = additions;
      configDeletions = deletions;
      break;
    default:
      otherAdditions = additions;
      otherDeletions = deletions;
      break;
  }
  return {
    codeAdditions,
...
async function main() {
  const pullRequests = await getPullRequests(gitHubAPIClient);
  const csvStream = fs.createWriteStream("repo_pr.csv");
  csvStream.write("PR Number,Title,Committer,Additions,Deletions\n");
  ...
  for (const pr of pullRequests) {
    const diff = await getPullRequestDiff(gitHubAPIClient, pr.number);
    const filename = pr.title;
    const diffCounts = parseDiff(diff || "", filename);
    const totalAdditions = diffCounts.codeAdditions + diffCounts.docAdditions + diffCounts.configAdditions + diffCounts.otherAdditions;
    const totalDeletions = diffCounts.codeDeletions + diffCounts.docDeletions + diffCounts.configDeletions + diffCounts.otherDeletions;
    csvStream.write(`${pr.number},"${pr.title.replace(/"/g, '\\"')}",${pr.user.login},${totalAdditions},${totalDeletions}
`);
  }
  csvStream.end();
}
