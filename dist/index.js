...
// src/service/diff.ts
function getCodeChangesLines(diff) {
  try {
    ...
    for (const line of lines) {
      ...
      if (trimmedLine.startsWith("diff --git") || trimmedLine.startsWith("index ") || trimmedLine.startsWith("--- ") || trimmedLine.startsWith("+++ ") || trimmedLine.startsWith("@@ ")) {
        continue;
      }
...
async function getOpenPullRequests(apiClient, owner, repo, startDate, endDate) {
  try {
    const response = await apiClient.getPullRequests(owner, repo);
    const repoPullRequests = [];
    for (const pr of response.unwrapOr([])) {
      await sleep(500);
      const diffResult = await apiClient.getPullRequestDiff(owner, repo, pr.number);
      const diffResponse = diffResult.unwrapOr("");
      const codeChanges = getCodeChangesLines(diffResponse);
  ...
  async getPullRequests(owner, repo, token) {
    try {
      const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/pulls?state=all`, {
        ...
        // Explicitly cast to HeadersInit
      });
      if (!response.ok) {
...
// src/index.ts
dotenv.config();
var repoUrl = process.env.GITHUB_REPO_URL;
...
async function main() {
  ...
  const repos = load(reposFile);
  const startDate = process.env.START_DATE || "2024-01-01";
  const endDate = process.env.END_DATE || "2025-12-31";
  for (const repoUrl2 of repos.repositories) {
    const urlParts = repoUrl2.split("/");
    const OWNER = urlParts[3];
    const REPO = urlParts[4];
    const pullRequests = await getOpenPullRequests(gitHubAPIClient, OWNER, REPO, startDate, endDate);
    if (!pullRequests || pullRequests.length === 0) {
      console.log(`No pull requests found for ${OWNER}/${REPO}.`);
      continue;
    }
    const csvContent = convertArrayToCsv(pullRequests);
    fs.writeFileSync(`@output/${REPO}.csv`, csvContent);
    console.log(`CSV file @output/${REPO}.csv generated successfully.`);
  }
  console.log("CSV file 'repo_pr.csv' generated successfully.");
}
main();
